...
def StripNonletters(word):
    while len(word) and word[-1] not in "abcdefghijklmnopqrstuvwxyz0123456789":
        word = word[:-1]
    while len(word) and word[0] not in "abcdefghijklmnopqrstuvwxyz0123456789":
        word = word[1:]
    return word
...
def GetResults(stats):
    ...
    return (fog, ari, cl, fkre, fkgl, smog, forc)
